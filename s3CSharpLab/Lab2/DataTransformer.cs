...
namespace Lab2
{
    ...
    public static class DataTransformer
    {
        ...
        public static readonly string InputBucketName = "jayarajawsinput20201209";

        // TODO 2: Set output bucket name (must be globally unique)
        public static readonly string OutputBucketName = "jayarajawsoutput20201209";
        ...
        {
            Debug.WriteLine("// Jayaraj's code . Create's S3 Client object and returns it.");
        ...
        private static GetObjectResponse GetS3Object(AmazonS3Client s3Client, string bucketName, string fileKey)
        {
            ...
            Debug.WriteLine("// Jayaraj's Code. Retreiving S3 objects");
        ...
        private static void PutObjectBasic(string bucketName, string fileKey, string transformedFile)
        {
            ...
            Debug.WriteLine("// JAYARAJ'S code . Put ");
            var putRequest = new PutObjectRequest();
            putRequest.Key = fileKey;
            putRequest.BucketName = bucketName;
            putRequest.ContentBody = transformedFile;
        ...
        private static string GeneratePresignedURL(string objectKey)
        {
            ...
            Debug.WriteLine("Jayaraj's Code. Generating Presigned Url");
            var req = new GetPreSignedUrlRequest();
            req.BucketName = OutputBucketName;
            req.Key = objectKey;
            req.Protocol = Protocol.HTTP;
            req.Verb = HttpVerb.GET;
            req.Expires = DateTime.Now.AddSeconds(900); // 15 minutes
            return s3ForStudentBuckets.GetPreSignedURL(req);
        }
        ...
        private static void PutObjectEnhanced(string bucketName, string fileKey, string transformedFile)
        {
            ...
            Debug.WriteLine("// JAYARAJ'S code . Put Enhanced ");
            var putRequest = new PutObjectRequest();
            putRequest.Key = fileKey;
            putRequest.BucketName = bucketName;
            putRequest.ContentBody = transformedFile;
            putRequest.ServerSideEncryptionMethod = ServerSideEncryptionMethod.AES256;
            //putRequest.TagSet.Add(new Tag { Key = "Environment", Value = "Development" });
            //putRequest.TagSet.Add(new Tag { Key = "Project", Value = "AWS Training by Sylvian" });

            putRequest.Metadata.Add("Contact", "Jayaraj Sivadasan");
            s3ForStudentBuckets.PutObject(putRequest);
        }
    }
}
